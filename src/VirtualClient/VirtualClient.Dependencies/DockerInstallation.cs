    ...
    public class DockerInstallation : VirtualClientComponent
    {
        ...
        private async Task DockerInstallInUbuntuAsync(EventContext telemetryContext, CancellationToken cancellationToken)
        {
            ...
            if (!isDockerInstalled)
            {
                ...
                await this.ExecuteCommandAsync(updateAptPackageCommand, null, Environment.CurrentDirectory, telemetryContext, cancellationToken, true)
                    .ConfigureAwait(false);

                await this.ExecuteCommandAsync(requiredPackagesCommand, null, Environment.CurrentDirectory, telemetryContext, cancellationToken, true)
                    .ConfigureAwait(false);

                await this.ExecuteCommandAsync("mkdir -p /etc/apt/keyrings", null, Environment.CurrentDirectory, telemetryContext, cancellationToken, true)
                    .ConfigureAwait(false);

                await this.ExecuteCommandAsync(addOfficialGPGKeyCommand, null, Environment.CurrentDirectory, telemetryContext, cancellationToken, true)
                    .ConfigureAwait(false);

                await this.ExecuteCommandAsync(setUpRepositoryCommand, null, Environment.CurrentDirectory, telemetryContext, cancellationToken, true)
                    .ConfigureAwait(false);

                await this.ExecuteCommandAsync(updateAptPackageCommand, null, Environment.CurrentDirectory, telemetryContext, cancellationToken, true)
                    .ConfigureAwait(false);

                await this.ExecuteCommandAsync(this.installDockerCommand, null, Environment.CurrentDirectory, telemetryContext, cancellationToken, true)
                    .ConfigureAwait(false);
        ...
        {
            var process = await this.ExecuteCommandAsync("bash -c \"docker --version\"", null, Environment.CurrentDirectory, telemetryContext, cancellationToken, true)
                .ConfigureAwait(false);
